...
Contas são fornecidas por seed no MVP; cadastro e recuperação de senha não são necessários. Tentativas de login malsucedidas são limitadas por e-mail e origem, antes da verificação do hash, e toda resposta carrega `X-Content-Type-Options`, `Referrer-Policy` e `X-Frame-Options`, definidos em `onRequest` para alcançarem também o stream SSE (ADR-026).
